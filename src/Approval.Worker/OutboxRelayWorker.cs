    ...
    private async Task ProcessPendingOutboxMessagesAsync(CancellationToken ct)
    {
        ...
        foreach (var msg in pendingList)
        {
            using (LogContext.PushProperty("TraceId", msg.TraceId))
            {
                ...
                try
                {
                    if (msg.EventType is BusinessConstants.OutboxEvents.InstanceApproved or BusinessConstants.OutboxEvents.InstanceRejected or BusinessConstants.OutboxEvents.InstanceReturned or "WorkflowStarted")
                    {
                        using var doc = JsonDocument.Parse(msg.PayloadJson);
                        var root = doc.RootElement;
                        var companyId = root.GetProperty("CompanyId").GetString() ?? BusinessConstants.DefaultCompanyDb;
                        ...
                        if (msg.EventType == BusinessConstants.OutboxEvents.InstanceApproved)
                        {
